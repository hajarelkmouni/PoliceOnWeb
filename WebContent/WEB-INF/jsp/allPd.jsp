...
<div class="aa" style="margin-top:50px;width:auto;">
     ...
     <a href="allPdVisiteur" style="height:35px;width:10%;font-size:15px;margin-left:940px;margin-bottom:-142px"class="btn btn-warning"><i class="fas fa-home"> Accueil</i></a> 
	 <form action="findPdVisiteur" method="POST" >
...
<c:forEach items = "${listpd}" var="personne" varStatus="boucle">
	...
	<div class="gallery">
		<h1><c:if test="${not empty bs}"><img src='data:image/jpeg;base64,${bs}'></c:if><br><br>${personne.getNom() } ${personne.getPrenom() }-${personne.getCIN_pd() }</h1>
	  <div class="desc">Agé(e) de ${personne.getAge() } ans,disparu(e) à ${personne.getLieudisparition() } le ${personne.getDatedisparition() }.${personne.getDescription() }</div>
	  <a href="temoignage?id=${personne.getCIN_pd() }">Faire un  témoignage <i class="fas fa-angle-right"></i></a>
	</div>
	</c:forEach>
</div>
</body>
