...
<div class="aa" style="margin-top:50px;width:auto;">
     ...
     <a href="allOwnerRVisiteur" style="height:35px;width:10%;font-size:15px;margin-left:940px;margin-bottom:-142px"class="btn btn-warning"><i class="fas fa-home"> Accueil</i></a> 
	 <form action="findOwnerVisiteur" method="POST" >
...
<c:forEach items = "${listobj}" var="personne" varStatus="boucle">
	...
	<div class="gallery">
		<h1><c:if test="${not empty bs}"><img src='data:image/jpeg;base64,${bs}'></c:if><br><br>${personne.getDate() }-${personne.getLieu() }</h1>
	  <div class="desc">${personne.getDescription() }</div>
	  <a href="temoignage?id=${personne.getId_obj() }">Faire un  témoignage <i class="fas fa-angle-right"></i></a>
	</div>
	</c:forEach>
</div>
</body>
