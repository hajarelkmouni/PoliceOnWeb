...
public class findDeclarationPr extends HttpServlet {
	private static final long serialVersionUID = 1L;
       

    public findDeclarationPr() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		DAODeclarationRecherche objPr = DAOFactory.getInstance().getDAODeclarationRecherche();
		DAODeclarationDisparition objPd = DAOFactory.getInstance().getDAODeclarationDisparition();
		String searchDr = request.getParameter("searchDR");
		List<DeclarationRecherche> listdr = objPr.findDeclarationPr(searchDr);
		List<DeclarationDisparition> listdd = objPd.getAll();
